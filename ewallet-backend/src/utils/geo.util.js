 ...
 * so the API can resolve a location. no effect in production.
 *
 * @param {string} ip
 * @returns {string}
 */
function resolveIp(ip) {
    ...
    // Strip the IPv6-mapped prefix on teh standard IPv4
    const cleanIp = ip.replace(/^::ffff:/, '');

    // Check if IP belongs to localhost or private network
